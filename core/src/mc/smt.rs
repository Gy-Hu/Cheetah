...
pub const BITWUZLA_CMD: SmtSolverCmd = SmtSolverCmd {
    ...
    args: &["--smt2", "--incremental", "--sat-engine=kissat"],
    supports_uf: false,
...
impl SmtModelChecker {
    ...
    pub fn check(
        ...
        let mut results: Vec<PropertyCheckResult> = Vec::new();
        let mut sat_properties = HashSet::<usize>::new(); // HashSet to store indices of SAT properties
            ...
            if self.opts.check_bad_states_individually { //loop for properties checking
                ...
                for (_bs_id, (expr_ref, _)) in bad_states.iter().enumerate() {
                    ...
                    if res == smt::Response::Sat {
                        ...
                        sat_properties.insert(_bs_id as usize); // Add the SAT property index to the set
                        results.push(PropertyCheckResult::Sat(_bs_id as u32, wit));
                    } else {
                        results.push(PropertyCheckResult::Unsat(_bs_id as u32));
                    }
                ...
                if res == smt::Response::Sat {
                    ...
                    results.push(PropertyCheckResult::Sat(0, wit));
                }
                check_assuming_end(&mut smt_ctx, &self.solver)?;
            }
